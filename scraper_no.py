"""
Scraper for proff.no (Norwegian business directory).

proff.no is a Next.js SSR app. Each page's __NEXT_DATA__ script contains
company listings including email and website under:
  pageProps.hydrationData.searchStore.companies.companies

URL: https://www.proff.no/_next/data/{buildId}/search.json?q={query}&geo={city}&lang=no&page={N}
Build ID is fetched dynamically from the homepage.

Coverage: the first page (premium/paid advertisers) has ~80% email rate.
Subsequent pages contain registry-only companies with sparse contact info.
"""
...
log = logging.getLogger("scraper_no")
...
MAX_RETRIES = 4
MAX_PAGES = 20  # cap to avoid scraping thousands of low-quality registry pages

_SKIP_DOMAINS = (
    "proff.no", "google.", "facebook.", "instagram.", "youtube.",
...
async def _get_build_id(session: aiohttp.ClientSession, semaphore: asyncio.Semaphore) -> str | None:
    """Fetch the current Next.js build ID from the proff.no homepage."""
    from scraper import _get_proxy, _block_proxy, _get_delay
    ...
    try:
        ...
        log.warning("Could not fetch build ID: %s", e)
    return None


def _extract_companies(data: dict) -> list[dict]:
    """Pull company records from Next.js page data."""
    store = (
        data.get("pageProps", {})
        .get("hydrationData", {})
        .get("searchStore", {})
...
async def scrape_category(city: str, category: str) -> list[dict]:
    """
    Scrape proff.no for a given city and category (Norwegian search term).

    Args:
        city:     Norwegian city name (e.g. "Oslo", "Bergen")
        category: Norwegian search term (e.g. "restaurant", "frisør")
    """
    ...
    log.info("Scraping NO %s/%s — concurrency=%d, proxies=%d", city, category, concurrency, proxy_count)
    ...
    async with aiohttp.ClientSession(connector=connector, cookie_jar=jar) as session:
        ...
        if not build_id:
            log.error("Could not determine proff.no build ID — aborting")
            return []

        log.debug("proff.no build ID: %s", build_id)

        def _api_url(page: int) -> str:
            import urllib.parse
            q = urllib.parse.quote(category)
            g = urllib.parse.quote(city)
            return f"{BASE_URL}/_next/data/{build_id}/search.json?q={q}&geo={g}&lang=no&page={page}"

        # Page 1
        data = await _fetch(session, _api_url(1), semaphore)
        if not data:
            log.warning("No response for NO %s/%s page 1", city, category)
            return []

        listings = _extract_companies(data)
        total_pages = min(_get_total_pages(data), MAX_PAGES)

        log.info("NO %s/%s — page 1/%d → %d listings", city, category, total_pages, len(listings))
        ...
        if total_pages > 1:
            for batch_start in range(2, total_pages + 1, concurrency):
                ...
                for page_num, page_data in zip(batch, pages_data):
                    ...
                    log.debug("NO %s/%s page %d → %d listings", city, category, page_num, len(page_listings))
    ...
    log.info("Finished NO %s/%s — %d results", city, category, len(results))
    return results
